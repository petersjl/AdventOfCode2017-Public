...
def main(test = false)
    ...
    if test
        ...
        puts "In 40M iterations there were #{solution} matches"
    end
    end_time = Time.now
    end_time = Time.now
...
def solve_puzzle(input)
    ...
    return run_iterations(input[0], input[1], 40000000)
end
...
def generate_next_pair(a, b)
    return [(a * 16807) % 2147483647, (b * 48271) % 2147483647] # multiply by the factors given and then take remainder
end

def run_tests()
    assert_equal(true, check_equality(245556042, 1431495498))
    assert_equal([1092455, 430625591], generate_next_pair(65, 8921))
    assert_equal(1, run_iterations(65, 8921, 5, true))
    assert_equal(588, run_iterations(65, 8921, 40000000))
