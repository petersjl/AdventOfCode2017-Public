...
def main(test = false)
    ...
    if test
        ...
        puts "The furthest the child got is #{solve_puzzle()} steps away"
    end
    end_time = Time.now
    end_time = Time.now
...
def solve_puzzle(input = nil)
    ...
    return find_max_dist(input)
end

def find_max_dist(directions)
    x = 0
    y = 0
    max_dist = 0
...
def run_tests()
    ...
    assert_equal(2, solve_puzzle("ne,ne,sw,sw"))
    assert_equal(2, solve_puzzle("ne,ne,s,s"))
    assert_equal(3, solve_puzzle("se,sw,se,sw,sw"))
    assert_equal(2, solve_puzzle("ne,ne,s,s,nw,nw"))
