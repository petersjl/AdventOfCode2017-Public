...
def solve_puzzle()
    ...
    for pair in input do
        ...
        if con.eval((registers[con.reg] or 0))
            new_val = ins.eval((registers[ins.reg] or 0))
            registers[ins.reg] = new_val
            greatest = new_val if new_val > greatest
        end
    end
    puts "The largest value ever held in a register is #{greatest}"
end
